...
int poll_devices()
{
  ...
  while (h != NULL)	/* one entire pass */
  {
    ...
    //writing json files
    json_init(get_bare_prognm(), "snipsportmon");
    //
	...
	fprintf (stderr, "%s: Error in checkports(), skipping device %s\n",
      ...
      else
      {
          ...
          if ((harray[i])->hname)
          {
			  ...
			  float val1 = v.loglevel;
				float val2 = floor(val1 + 0.5);
                fprintf(stderr, "%lld elapsedsecs, val = %.2f, loglevel = %.2f \n",    harray[i]->elapsedsecs, val, val2);
